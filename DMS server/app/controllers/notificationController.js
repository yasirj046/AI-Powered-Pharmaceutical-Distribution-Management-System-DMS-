...
const notificationController = {
  ...
  async sendPasswordResetOTP(req, res) {
    try {
      ...
      // Send OTP email
      const result = await notificationService.sendPasswordResetOTP(
        email,
        otp,
        user.name
      );
    ...
    try {
      ...
      if (user.passwordReset.otp !== otp) {
        ...
        return res.status(400).json({
          ...
          attemptsRemaining: 5 - user.passwordReset.attempts
        });
      }

      // OTP is valid, reset password
      user.password = newPassword; // Will be hashed by pre-save middleware
      user.passwordReset = undefined; // Clear password reset data
      await user.save();
